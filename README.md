...
В ходе работ нам удалось достичь **0.91 MRR**, это означает что среднее значение позиции правильного ответа приблизительно равно **1.095**. Вероятность того, что правильный ответ окажется **на первом месте** рекомендаций приблизительно равно **87%**, **95%** что войдёт в **топ 3**, **98%** - в **топ 10**. Для этого мы дообучали на известных взаимодействиях берт-дистиллят https://huggingface.co/cointegrated/rubert-tiny2 . Финальная модель сохранена в репозитории https://huggingface.co/micoff/rubert-tiny2-tuned-for-prosept . 
